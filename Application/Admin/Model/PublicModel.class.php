...
class PublicModel extends Model
{
	public function auth($datas)
	{
		...
		if (check_verify($_POST["verify_code"]) == false) {
			//return(array("status" => 0, "info" => "验证码错误啦，再输入吧","type"=>"verifycode"));
		}
		*/
		$M = M("Admin");


		if (1 <= $M->where("`email`='" . $datas["email"] . "' or nickname ='".$datas['email']."'")->count()) {
			$info = $M->where("`email`='" . $datas["email"] . "' or nickname ='".$datas['email']."'")->find();
			...
			if ($info["pwd"] == encrypt($datas["pwd"])) {
				...
				return array("status" => 1, "info" => "登录成功", "url" => u("Index/index"),"email"=>$info['email']);
			}
			else {
	...
	public function findPwd()
	{
		...
		if (check_verify($_POST["verify_code"]) == Model\false) {
			exit(json_encode(array("status" => 0, "info" => "验证码错误啦，再输入吧")));
		}
