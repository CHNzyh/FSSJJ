...
require 'config.php';
$filepath = $_GET['path'];
//$_SESSION['wf_uroot'] = wf_config('ROOT_PATH').'/'.$filepath;
if (!WF_Auth::isLogin($filepath)) {
...
<script id="nlist-path" type="text/template">
	<span id="list_head_left"></span>
	<span id="list_head_center">当前目录：{@each paths as dir}<a href="javascript:app.nfs.nlist('${dir.path}');">${dir.name}/</a>{@/each}(<? echo $_SESSION['wf_uhost'];?>)</span>
	<span id="list_head_right"></span>
</script>
...
<script id="nlist-list-main" type="text/template">
	...
	<tr class="nlist-list">
		...
		<td>文件大小</td>
		<td>文件权限</td>
		<td align="center" colspan="4">相关操作</td>
	</tr>
	...
	{@each list.files as file,idx}
		<tr class="file-list-${idx}">
			...
			<td><a href="javascript:app.nfs.url('${file.path}', '${file.name}', 'file')">地址</a></td>
...
$(function () {
	...
	app.mui.init();
	app.nfs.init();
	//app.nfs.nlist('/Finance/OldDataExplain/');
});
</script>
</body>
</html>